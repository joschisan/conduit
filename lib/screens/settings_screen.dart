...
class SettingsScreen extends StatefulWidget {
  final InitializedDatabase initializedDb;

  const SettingsScreen({super.key, required this.initializedDb});
...
class _SettingsScreenState extends State<SettingsScreen> {
          ...
          children: [
            ...
            Expanded(
              child: FutureBuilder<List<FederationInfo>>(
                future: widget.initializedDb.listFederations(),
  ...
  void _showScannerDrawer() {
    showStandardDrawer(
      ...
      child: InviteScannerWidget(
        initializedDb: widget.initializedDb,
  ...
  TaskEither<String, void> _handleJoinFederation(InviteCodeWrapper invite) {
    return safeTask(() async {
      final client = await widget.initializedDb.join(invite: invite);
  ...
  TaskEither<String, void> _handleRecoverFederation(InviteCodeWrapper invite) {
    return safeTask(() async {
      final client = await widget.initializedDb.recover(invite: invite);
  ...
  Future<void> _handleFederationTap(FederationInfo federation) async {
    try {
      final client = await widget.initializedDb.load(
  ...
  TaskEither<String, void> _handleLeaveFederation(FederationId federationId) {
    return safeTask(
      () => widget.initializedDb.leave(federationId: federationId),
    );
  }
  ...
  Future<void> _handleSeedPhraseTap() async {
    try {
      ...
      final seedPhrase = await widget.initializedDb.seedPhrase();
