...
use fedimint_eventlog::EventLogEntry;
...
use crate::db::EventLogEntryKey;
use crate::db::EventLogEntryPrefix;
...
pub fn parse_lnurl(request: &str) -> Option<LnurlWrapper> {
        ...
        .any(|pattern| Regex::new(pattern).unwrap().is_match(request))
    {
        ...
        return LightningAddress::from_str(&address)
            .ok()
            .map(|address| LnurlWrapper(address.endpoint()));
    }
...
impl ConduitClient {
    ...
    pub async fn subscribe_event_log(&self, sink: StreamSink<ConduitEvent>) {
        ...
        let history = self
            ...
            .await
            .collect::<Vec<(EventLogEntryKey, EventLogEntry)>>()
            .await;
...
fn parse_event_log_entry(entry: &EventLogEntry) -> Option<ConduitEvent> {
